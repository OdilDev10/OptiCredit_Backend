...
async def list_lender_products(
    ...
    """List all loan products for the lender."""
    service = LoanProductService(session)
    products = await service.get_lender_products(
        UUID(lender_id),
        active_only=active_only,
    )
    total = len(products)
    items = [_product_to_response(p) for p in products[skip : skip + limit]]
    return PaginatedProductsResponse(items=items, total=total, skip=skip, limit=limit)
